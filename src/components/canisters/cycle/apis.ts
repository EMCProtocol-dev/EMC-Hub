...
// 获取cycles余额
export const getWalletBalance = async (agent: Agent, value: Principal): Promise<bigint> => {
...
// 转账
export const transfer = async (agent: Agent, pid: Principal, amount: bigint): Promise<TxReceipt> => {
  const actor = getActorByAgent(agent);
