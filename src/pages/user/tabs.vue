    ...
    <NCollapse :trigger-areas="['main']" :accordion="true" arrow-placement="right" :expanded-names="currentName">
      <template v-for="item in tabs">
        <NCollapseItem :name="item.name" style="margin: 0">
          <template #header>
            <div :class="['tabs-nav-item', currentKey === item.path ? 'tabs-nav-item__active' : '']" @click="onPressItem(item)">
          ...
          <template v-for="sub in item.children">
            <div :class="['tabs-nav-children', currentKey === sub.path ? 'tabs-nav-item__active' : '']" @click="onPressItem(sub)">
              <span>{{ sub.label }} </span>
            </div>
          </template>
        </NCollapseItem>
