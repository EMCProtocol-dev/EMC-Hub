...
// 初始化钱包
export const InitWallet = async (principal): Promise<WalletType> => {
  return await LoadBalance(principal);
};

// 创建钱包实例
export const LoadBalance = async (principal): Promise<WalletType> => {
...
// 加载交易记录
export const LoadTransaction = async (address) => {
  if (!address) {
    return;
  }
  let paramet;
  ...
  try {
    const transactions: any = await rosettaApi.getTransactionsByAccount(
      paramet
    );
...
// 通过pid获取地址
export const GetAddress = (pid): string => {
  if (!pid) {
    return "";
  }
