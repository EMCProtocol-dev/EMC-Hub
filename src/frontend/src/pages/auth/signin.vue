<template>
  <NTabs class="card-tabs" default-value="signin" size="large" animated :value="current" @update:value="onChangeTab" tab-style="display:none;" pane-style="padding-top:0">
    <NTabPane name="signin-wallet" tab="ICP Wallet">
      ...
      <NButton class="tab-toggle" quaternary type="primary" :disabled="business" strong size="large" @click="current = 'signin-account'">Password Sign In</NButton>
    </NTabPane>
    <NTabPane name="signin-account" tab="Account">
      ...
      <NButton class="tab-toggle" quaternary type="primary" :disabled="business" strong size="large" @click="current = 'signin-wallet'">Wallet Sign In</NButton>
    </NTabPane>
  </NTabs>
</template>
<script lang="ts">
